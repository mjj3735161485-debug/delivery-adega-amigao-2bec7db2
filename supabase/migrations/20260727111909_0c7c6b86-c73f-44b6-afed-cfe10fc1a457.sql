...
BEGIN
  ...
  -- Aplica cashback: só para usuários logados, limitado ao saldo e ao SUBTOTAL (nunca desconta taxa de entrega)
  IF v_cashback_req > 0 AND v_uid IS NOT NULL THEN
    SELECT COALESCE(SUM(CASE WHEN tipo = 'debito' THEN -valor ELSE valor END), 0)
      ...
      WHERE user_id = v_uid;
    v_cashback_aplicado := LEAST(v_cashback_req, v_cashback_saldo, sub);
    IF v_cashback_aplicado < 0 THEN v_cashback_aplicado := 0; END IF;
    tot := GREATEST(tax, tot - v_cashback_aplicado);
  END IF;
